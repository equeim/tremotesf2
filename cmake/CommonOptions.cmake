...
macro(apply_hardening_options common_compile_options_var common_compile_definitions_var)
    if (NOT MSVC)
        ...
        check_cxx_compiler_flag("-fhardened" TREMOTESF_COMPILER_SUPPORTS_FHARDENED)
        ...
        if (TREMOTESF_COMPILER_SUPPORTS_FHARDENED)
            list(APPEND "${common_compile_options_var}" -fhardened -Wno-hardened -U_FORTIFY_SOURCE)
            ...
            list(APPEND "${common_compile_options_var}" -ftrivial-auto-var-init=pattern -fstack-protector-strong -U_FORTIFY_SOURCE)
            if (CMAKE_SYSTEM_NAME MATCHES "Linux|FreeBSD")
                list(APPEND "${common_compile_options_var}" -fstack-clash-protection)
            endif()
            if ((VCPKG_TARGET_TRIPLET MATCHES "^x64.*") OR (NOT DEFINED VCPKG_TARGET_TRIPLET AND CMAKE_SYSTEM_PROCESSOR MATCHES "x86_64|AMD64"))
                list(APPEND "${common_compile_options_var}" -fcf-protection=full)
            endif()
            # -mbranch-protection=standard causes crashes on macOS
            if (NOT APPLE AND ((VCPKG_TARGET_TRIPLET MATCHES "^arm64.*") OR (NOT DEFINED VCPKG_TARGET_TRIPLET AND CMAKE_SYSTEM_PROCESSOR MATCHES "arm64|ARM64")))
                list(APPEND "${common_compile_options_var}" -mbranch-protection=standard)
            endif()
            if (NOT (TREMOTESF_ASAN OR CMAKE_CXX_FLAGS MATCHES "-fsanitize=address"))
                list(APPEND "${common_compile_options_var}" $<$<NOT:$<CONFIG:Debug>>:-D_FORTIFY_SOURCE=3>)
            endif()
