...
namespace tremotesf {
    ...
    QDebug operator<<(QDebug debug, const MagnetLink& magnetLink) {
        debug << fmt::format(impl::singleArgumentFormatString, magnetLink);
        return debug;
    }
...
fmt::formatter<tremotesf::MagnetLink>::format(const tremotesf::MagnetLink& magnetLink, format_context& ctx) const {
