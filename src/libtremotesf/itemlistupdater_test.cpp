...
protected:
    typename std::vector<Item>::iterator findNewItemForItem(std::vector<Item>& container, const Item& item) override {
        return std::find_if(container.begin(), container.end(), [&item](const Item& newItem) { return newItem.id == item.id; });
    };
    ...
    void onRemovedItems(size_t first, size_t last) override {
        removedIndexRanges.emplace_back(first, last);
    }
    ...
    void onChangedItems(size_t first, size_t last) override {
        changedIndexRanges.emplace_back(first, last);
    }

    Item createItemFromNewItem(Item&& newItem) override {
...
class ItemListUpdaterTest : public QObject {
    Q_OBJECT
private slots:
    void emptyListDidNotChange() {
        checkUpdate({}, {});
    }

    void singleItemListDidNotChange() {
        checkUpdate({{42, "666"}},
                    {{42, "666"}});
    }

    void multipleItemsListDidNotChange() {
        checkUpdate({{42, "666"}, {1, "Foo"}, {11, "Bar"}},
                    {{42, "666"}, {1, "Foo"}, {11, "Bar"}});
    }

    void addedSingleItemToEmptyList() {
        checkUpdate({},
                    {{42, "666"}});
    }

    void addedMultipleItemsToEmptyList() {
        checkUpdate({},
                    {{42, "666"}, {1, "Foo"}});
    }

    void addedSingleItemToNonEmptyList() {
        checkUpdate({{42, "666"}},
                    {Item{42, "666"}, {1, "Foo"}});
    }

    void addedMultipleItemsToNonEmptyList() {
        checkUpdate({{42, "666"}},
                    {{42, "666"}, {1, "Foo"}, {11, "Bar"}});
    }

    void removedItemFromSingleItemList() {
        checkUpdate({{42, "666"}},
                    {});
    }

    void removedLastItemFromMultipleItemsList() {
        checkUpdate({{42, "666"}, {666, ""}},
                    {{42, "666"}});
    }

    void removedFirstItemFromMultipleItemsList() {
        checkUpdate({{42, "666"}, {666, ""}},
                    {{666, ""}});
    }

    void removedMiddleItemFromMultipleItemsList() {
        checkUpdate({{42, "666"}, {666, ""}, {1, ""}},
                    {{42, "666"}, {1, ""}});
    }

    void removedTwoConsecutiveItemsFromEnd() {
        checkUpdate({{42, "666"}, {666, ""}, {1, ""}, {18, "Nope"}},
                    {{42, "666"}, {666, ""}});
    }
    ...
    void removedTwoSeparateItems1() {
        checkUpdate({{42, "666"}, {666, ""}, {1, ""}, {18, "Nope"}, {77, "Foo"}},
                    {{42, "666"}, {666, ""}, {18, "Nope"}});
    }
...
private:
    ...
    void checkUpdateInner(const std::vector<Item>& oldList, const std::vector<Item>& newList) {
        ...
        QCOMPARE_THROW(std::set(directlyUpdatedList.begin(), directlyUpdatedList.end()), std::set(newList.begin(), newList.end()));

        auto indirectlyUpdatedList = oldList;

        for (const auto& [first, last] : updater.removedIndexRanges) {
            indirectlyUpdatedList.erase(indirectlyUpdatedList.begin() + static_cast<ptrdiff_t>(first), indirectlyUpdatedList.begin() + static_cast<ptrdiff_t>(last));
        }

        for (const auto& [first, last] : updater.changedIndexRanges) {
            std::copy(directlyUpdatedList.begin() + static_cast<ptrdiff_t>(first), directlyUpdatedList.begin() + static_cast<ptrdiff_t>(last), indirectlyUpdatedList.begin() + static_cast<ptrdiff_t>(first));
        }

        if (updater.addedCount) {
            indirectlyUpdatedList.reserve(indirectlyUpdatedList.size() + *updater.addedCount);
            std::copy(directlyUpdatedList.end() - static_cast<ptrdiff_t>(*updater.addedCount), directlyUpdatedList.end(), std::back_insert_iterator(indirectlyUpdatedList));
        }
...
QTEST_MAIN(ItemListUpdaterTest)
